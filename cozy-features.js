(function (global) {
  ...
  var state = { loaded: false, available: false, profiles: {}, requests: [], viewer: null, profileMember: "", profilePickerOpen: false, timeEditorOpen: false, expanded: false, marketTab: "public", adminPw: "", draft: null, formHistoryActive: false, saving: false, activityTimer: null };
  var SCHEDULE = [
  ...
  function renderActivity() {
    ...
    box.innerHTML = active.length ? active.map(function (name) { var all=(state.profiles[name].timeTags||[]).indexOf("종일반")>=0; return '<button type="button" class="activity-chip activity-member" data-activity-member="'+esc(name)+'">'+esc(name)+(all?' · 종일반':'')+'</button>'; }).join("") : '<span class="muted">이 시간대에 등록된 길드원이 없습니다.</span>';
  ...
  function renderMarket() {
    ...
    var admin='<div class="market-admin"><button type="button" class="feature-secondary" data-act="toggle-market-admin">'+(state.adminPw?'관리자 권한 해제':'관리자 권한 활성화')+'</button>'+(state.adminPw?'<span>관리자 권한 활성화됨</span>':'')+'</div>';
    document.getElementById("marketView").innerHTML='<div class="feature-panel"><div class="feature-head"><div class="feature-title"><button class="feature-back" data-act="home">← 뒤로</button><h2>거래소 의뢰</h2></div><button class="feature-primary" data-act="new-request">꽃 의뢰하기 →</button></div>'+note()+'<h3>이전 기록</h3><div class="history-row">'+hist+'</div><div class="market-list-head"><h3>거래소 의뢰 목록</h3>'+tabs+'</div><div class="market-groups">'+groups+'</div>'+admin+'</div>';
  }
  function requestCard(r) {
    var admin=Boolean(state.adminPw);
    var can = admin || (state.viewer && state.viewer.member && (state.viewer.member===r.requester || state.viewer.member===r.target) && r.canMutate !== false);
    var canEdit = admin || (state.viewer && state.viewer.member===r.requester && r.canEdit !== false && !r.completedAt);
  ...
  function openForm(seed) {
    state.saving=false; state.draft=freshDraft(seed);
    if (!state.formHistoryActive) {
      global.history.pushState({ cozyMarketForm: true }, "", global.location.href);
      state.formHistoryActive=true;
    }
    renderForm();
  }
  function clearForm() { state.formHistoryActive=false; state.draft=null; var overlay=document.querySelector('.market-form-overlay'); if(overlay)overlay.remove(); }
  function closeForm() { if(state.formHistoryActive&&global.history.state&&global.history.state.cozyMarketForm){global.history.back();return;}clearForm(); }
  ...
  function renderForm() {
    ...
    var chosen=d.items.map(function(i,idx){return '<div class="selected-flower">'+flowerThumb(i.flower)+'<b>'+esc(i.flower)+'</b><div class="qty-control"><button type="button" class="qty-step" data-qty-step="'+idx+'" data-delta="-1">−</button><input class="qty-input" data-qty="'+idx+'" inputmode="numeric" value="'+esc(i.quantity)+'" placeholder="수량"><button type="button" class="qty-step" data-qty-step="'+idx+'" data-delta="1">+</button></div><button type="button" data-remove="'+idx+'">×</button></div>';}).join("") || '<div class="feature-empty">꽃을 선택해 주세요.</div>';
  ...
  function submitDraft() {
    ...
    if(state.draft.editing){payload.id=state.draft.id;payload.actor=ownName();if(state.adminPw)payload.pw=state.adminPw;}
    api(payload).then(function(){if(!state.draft.editing)saveHistory({requester:payload.requester,target:payload.target,items:payload.items});closeForm();return loadShared();}).then(renderMarket).catch(function(e){state.saving=false;state.draft.error=e.message;renderForm();});
  }
  ...
  document.addEventListener("click", function (ev) {
    var act=ev.target.closest("[data-act]"); if(act){var a=act.getAttribute("data-act");if(a==="home"){showOnly("homeView");return;}if(a==="toggle-profile-picker"){state.profilePickerOpen=!state.profilePickerOpen;renderProfile();return;}if(a==="toggle-time-editor"){state.timeEditorOpen=!state.timeEditorOpen;renderProfile();return;}if(a==="expand"){state.expanded=!state.expanded;renderProfile();return;}if(a==="new-request"){openForm();return;}if(a==="toggle-target-picker"){state.draft.targetPickerOpen=!state.draft.targetPickerOpen;renderForm();return;}if(a==="toggle-market-admin"){if(state.adminPw){state.adminPw="";renderMarket();return;}var pw=prompt("관리자 비밀번호를 입력해 주세요.");if(pw===null)return;api({action:"market.verifyAdmin",pw:pw}).then(function(){state.adminPw=pw;renderMarket();}).catch(function(e){alert(e.message);});return;}if(a==="market-help"){document.body.insertAdjacentHTML('beforeend','<div class="market-help-overlay"><div class="market-help-dialog"><div class="market-help-title"><b>세레비 안내</b><button class="feature-secondary" data-act="close-market-help">닫기</button></div><p>세레비는 백야 길드원이 편하게 게임 초반 꽃을 구할 수 있도록 새우(부길드장)이 관리중인 계정입니다. 1일 1회, 인당 총 9개, 하루 3명씩 순서대로 게임 내 귓속말로 찾아뵈오니 편하게 신청 주세요.</p></div></div>');return;}if(a==="close-market-help"){ev.target.closest('.market-help-overlay').remove();return;}if(a==="close-form"){closeForm();return;}if(a==="submit"){submitDraft();return;}}
    var profileMember=ev.target.closest("[data-profile-member]");if(profileMember){var nextMember=profileMember.getAttribute("data-profile-member");if(typeof global.pickMe==="function")global.pickMe(nextMember);showProfile(nextMember);return;}
    var activityMember=ev.target.closest("[data-activity-member]");if(activityMember&&typeof global.openMemberModal==="function"){global.openMemberModal(activityMember.getAttribute("data-activity-member"));return;}
    ...
    var toggle=ev.target.closest("[data-toggle-request]");if(toggle){var togglePayload={action:"market.toggleComplete",id:toggle.getAttribute("data-toggle-request"),actor:ownName()};if(state.adminPw)togglePayload.pw=state.adminPw;api(togglePayload).then(loadShared).then(renderMarket).catch(function(e){alert(e.message);});}
  });
  ...
  global.addEventListener("popstate",function(){if(state.formHistoryActive)clearForm();});
