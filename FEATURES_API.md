...
- `{ "action": "market.create", "requester": "가재", "target": "", "items": [{"flower":"황금 장미","quantity":"1000"}] }` (`target`이 빈 문자열이면 지정 없음/아무나)
- `{ "action": "market.update", "id": "...", "actor": "현재 선택한 내 이름", "requester": "가재", "target": "새우", "items": [{"flower":"황금 장미","quantity":"1000"}] }`
- `{ "action": "market.toggleComplete", "id": "...", "actor": "현재 선택한 내 이름" }`
- `{ "action": "market.verifyAdmin", "pw": "관리자 비밀번호" }`

서버는 길드원 존재 여부, 대상자의 최신 보유 꽃, 서로 다른 꽃 1~4종, 중복 꽃 금지, 수량의 `/^[1-9]\d*$/` 조건을 다시 검사한다. 지정 없음은 전체 도감에 존재하는 꽃을 선택할 수 있다. 목록에 없는 꽃은 항목에 `"manual": true`를 넣고 1~50자의 이름으로 직접 등록할 수 있다. `꽃 미지정`도 직접 입력 항목으로 저장하며, 화면에서는 다른 꽃과 동시에 선택되지 않게 한다. 대상자가 `세레비`이면 한 카드의 모든 꽃 수량 합계가 9 이하여야 한다. 수정은 완료 전 카드의 기존 의뢰인만 가능하다. 수량은 JS 정수로 변환하지 않고 문자열 또는 충분한 정밀도의 정수로 보존한다. 생성 요청은 재시도 중복을 막을 수 있도록 향후 idempotency key를 받는 편이 안전하다.

관리자 비밀번호는 `market.verifyAdmin`으로 먼저 확인한다. 활성화된 화면은 `market.update`와 `market.toggleComplete` 요청에 `pw`를 함께 보내며, 서버는 비밀번호가 맞을 때 의뢰인·대상자 및 완료 여부와 관계없이 모든 카드의 수정과 완료 상태 변경을 허용한다. 비밀번호는 서버 응답이나 공용 저장소에 보관하지 않는다.
